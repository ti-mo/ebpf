...
func LoadCollectionSpecFromReader(rd io.ReaderAt) (*CollectionSpec, error) {
	...
	if ec.version == magicVersion {
		v, err := detectKernelVersion()
		if err != nil {
			return nil, xerrors.Errorf("detect kernel version: %w", err)
		}
		ec.version = v.KernelVersion()
	}
...
func findKernelVersion(s string) (string, error) {
	...
	if m == nil {
		return "", xerrors.Errorf("parsing kernel version from string: %s", s)
	}
...
func parseKernelVersion(s string) (internal.Version, error) {
	...
	if err != nil {
		return internal.Version{}, xerrors.Errorf("parsing into 'Major.Minor.Patch': %s", s)
	}
	return v, nil
}
